...
export const ActionList = memo(function ActionList({
  ...
  if (filteredActions.length === 0) {
    return (
      <div className="px-4 py-8 text-center text-sm text-muted-foreground/60">No actions found</div>
    )
  }
  ...
  return (
    <button
      ...
      className={cn(
        ...
        isSelected &&
          'bg-white/[0.08] dark:bg-white/[0.08] shadow-[inset_0_1px_0_0_rgba(255,255,255,0.04)]'
      )}
    >
      <div className="flex items-center gap-3">
        <span className="text-base w-5 h-5 flex items-center justify-center shrink-0">
          {action.icon}
        </span>
        <span className="text-[13px] font-medium text-foreground/90 group-hover:text-foreground transition-colors duration-200">
          {action.label}
        </span>
      </div>
      {action.shortcut && (
