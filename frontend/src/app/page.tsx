...
export default function Home() {
  ...
  useEffect(() => {
    if (typeof window !== 'undefined' && window.electron) {
      const storedTextOutputMode = localStorage.getItem('ai-keyboard-text-output-mode') as
        | 'paste'
        | 'typewriter'
        | 'typewriter-leetcode'
        | null
      const storedSuggestionMode = localStorage.getItem('ai-keyboard-suggestion-mode') as
        | 'hotkey'
        | 'auto'
        | null
      ...
      <main className="flex flex-col h-screen rounded-2xl border border-white/20 dark:border-white/10 bg-background/80 backdrop-blur-2xl shadow-2xl">
        ...
        <div className="flex-1 flex items-center justify-center">
          <div className="text-center text-muted-foreground">
            <p className="text-sm">
              Press <kbd className="rounded bg-muted px-2 py-1 font-mono text-xs">Ctrl+\</kbd>
            </p>
  ...
  return (
    <main className="h-screen overflow-hidden rounded-2xl border border-white/20 dark:border-white/10 bg-background/80 backdrop-blur-2xl shadow-2xl flex flex-col">
      ...
      <div className="flex-1 overflow-hidden">
        <ActionMenu selectedText={selectedText} onClose={handleClose} onReplace={handleReplace} />
      </div>
    </main>
  )
}
