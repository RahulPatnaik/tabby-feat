...
export async function getConversationMessages(conversationId: string): Promise<UIMessage[]> {
  const options = await createAuthenticatedFetchOptions({ method: 'GET' })
  const response = await fetch(getApiUrl(`/api/conversations/${conversationId}/messages`), options)
  const data = await response.json()
  if (!data.success) {
...
export async function deleteConversation(conversationId: string): Promise<{ success: boolean }> {
  ...
  const response = await fetch(getApiUrl(`/api/conversations/${conversationId}`), options)
  const data = await response.json()
  if (!data.success) {
...
export async function getChatById(conversationId: string): Promise<Conversation | null> {
  const options = await createAuthenticatedFetchOptions({ method: 'GET' })
  const response = await fetch(getApiUrl(`/api/conversations/${conversationId}`), options)
  const data = await response.json()
  if (!data.success) {
