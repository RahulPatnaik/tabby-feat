...
contextBridge.exposeInMainWorld('electron', {
  ...
  moveWindow: (delta: { x: number; y: number }) => ipcRenderer.send('move-window', delta),
  ...
  setTextOutputMode: (mode: 'paste' | 'typewriter' | 'typewriter-leetcode') =>
    ipcRenderer.send('set-text-output-mode', mode),
  ...
  onAnalyzeScreenshot: (callback: (data: { dataUrl: string; timestamp: string }) => void) => {
    const handler = (_: IpcRendererEvent, data: { dataUrl: string; timestamp: string }) =>
      callback(data)
    ipcRenderer.on('analyze-screenshot', handler)
    return () => ipcRenderer.removeListener('analyze-screenshot', handler)
  },
  openExternal: (url: string) => ipcRenderer.send('open-external', url),
  notifyAnalysisComplete: (success: boolean) => ipcRenderer.send('analysis-complete', success),
  ...
  setGhostTextAutoTrigger: (enabled: boolean) =>
    ipcRenderer.send('set-ghost-text-auto-trigger', enabled),
  getGhostTextAutoTriggerDelay: () => ipcRenderer.invoke('get-ghost-text-auto-trigger-delay'),
  setGhostTextAutoTriggerDelay: (delayMs: number) =>
    ipcRenderer.send('set-ghost-text-auto-trigger-delay', delayMs),
